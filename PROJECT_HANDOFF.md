...
├── dist/Agent Pilot_0.1.0_aarch64.dmg             # 当前已构建 dmg，未正式签名
...
```text
dist/Agent Pilot_0.1.0_aarch64.dmg
```
...
- `openButtonText()`：根据 target 类型显示“定位 Codex / 定位终端 / 查看进程 / 打开终端”。
...
远程 tmux 状态读取需要后台 SSH 免密可访问。否则无法无感读取远程 pane 输出。

### 9.4 Ghostty 定位限制

Terminal.app 可以根据进程/TTY 更稳定地定位已有窗口；Ghostty 暂未找到可靠的“根据 PID 激活已有 window/pane”的通用方案。当前保留 fallback。
